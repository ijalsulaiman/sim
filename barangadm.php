...
include 'koneksi.php';
  ...
  if(isset($_POST['tambah'])){
    ...
    $tambah = mysqli_query($koneksi,"insert into barang (id_barang, nama_barang, keterangan) values('$id_barang','$nama_barang','$keterangan')");
    if (!$tambah) {
      echo "<script>alert('Pastikan semua kolom terisi dengan benar! ');window.location='barangadm.php';</script>;";
    }else{
      $tambahpersediaan = mysqli_query($koneksi,"insert into persediaan_barang (id_persediaan, id_barang, stok, satuan, id_gudang,lokasi, safety_stock) values('','$id_barang','$stok','$satuan','$id_gudang','$lokasi','1')");
    $tambahperencanaan = mysqli_query($koneksi,"insert into perencanaan (id_perencanaan, id_barang, jumlah) values('','$id_barang','1')");
      echo "<script>alert('Data Berhasil Ditambahkan');window.location='barangadm.php';</script>;";
    }
}
if(isset($_POST['updatebarang'])){
    ...
    $updatebarang = mysqli_query($koneksi,"update barang set nama_barang='$nama_barang', keterangan='$keterangan' where id_barang='$id_barang'");
     if (!$updatebarang) {
      echo "<script>alert('Pastikan semua kolom terisi dengan benar! ');window.location='barangadm.php';</script>;";
    }else{
      echo "<script>alert('Data Berhasil Diubah');window.location='barangadm.php';</script>;";
    }
   }
   if (isset($_POST['delbarang'])) {
    $id_barang= $_POST['id_barang'];

    $delbarang = mysqli_query($koneksi,"delete from barang where id_barang='$id_barang'");
  }
  ?>
  <!-- Preloader -->
  ...
  <div class="content-wrapper">
    ...
    <h1 align="center">DAFTAR BARANG</h1>
      ...
      <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#tambahpersediaan">
 <img src="tambah.png" width="20px" height="20px">Tambah
</button>

<!-- Modal -->
<div class="modal fade" id="tambahpersediaan" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="tambahpersediaan">Tambah Data Barang</h5>
      ...
      <form method="POST">
          <div class="form-group" align="left">
            ...
            <input type="text" class="form-control" name="id_barang" placeholder="Masukkan Kode Barang" required>            
          </div>    
          <div class="form-group" align="left">
          ...
          <div class="form-group" align="left">
            ...
            <input type="number" class="form-control" name="stok" min="0" placeholder="Masukkan stok Persediaan" required>       
          </div>
          <div class="form-group" align="left">
           ...
           <div class="form-group" align="left">
            <label for="text">Pilih Gudang</label>
            <select name="id_gudang" class="form-control" required>
              <option> Pilih Gudang</option>
              <?php
                  $ambilids = mysqli_query($koneksi,"select * from gudang");
                  while ($fetcharray = mysqli_fetch_array($ambilids)) {
                    $kdgudang = $fetcharray['id_gudang'];
                    $nama_gudang = $fetcharray['nama_gudang'];
                  ?>
                  <option value="<?=$kdgudang;?>"><?=$nama_gudang;?></option>
                  <?PHP 
                  }
                  ?>
            </select>                 
    ...
    <table id="example2" class="table table-bordered table-hover dataTable dtr-inline w-75 mx-auto" aria-describedby="example2_info">
          ...
          <tbody>
            ...
            <tr>
                ...
                <td><?=$nama_barang;?></td>
                <td><?=$keterangan;?></td>
          <td><button class=" btn btn-secondary" data-bs-toggle="modal" data-bs-target="#edit<?=$id_barang;?>"><img src="edit.png" width="20px" height="20px"></button>&emsp;<button class="btn btn-danger"data-bs-toggle="modal" data-bs-target="#delete<?=$id_barang;?>"><img src="hapus.png" width="20px" height="20px"></button></td>
          </td>
            </tr>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="persediaan">Ubah Data Barang Persediaan</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
      <form method="POST">
