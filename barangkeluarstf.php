...
include 'koneksi.php';
  ...
  if($_SESSION['level']=="adm"){
    echo "<script>alert('Anda Tidak Memiliki Akses');window.location='barangkeluaradm.php';</script>;";
  }
  ...
  if(isset($_POST['brgkeluar'])) {
    ...
    {
       echo "<script>alert('Pastikan semua kolom terisi dengan benar! ');window.location='barangkeluarstf.php';</script>;";
    }
    else{
    ...
    if (!$delmas) {
      echo "<script>alert('Data tidak terhapus ');window.location='barangkeluarstf.php';</script>;";
    }else{
     $ambildatass = mysqli_query($koneksi,"select barang_masuk.id_barang, IFNULL(round(0.524*SQRT((avg(detailbarangmasuk.lead)*(STDDEV_SAMP(barang_keluar.jumlah)*STDDEV_SAMP(barang_keluar.jumlah)))+((avg(barang_keluar.jumlah)*(avg(barang_keluar.jumlah))*(stddev_samp(detailbarangmasuk.lead)*stddev_samp(detailbarangmasuk.lead)))))),1) as ss from barang_masuk inner join detailbarangmasuk on detailbarangmasuk.id_barang=barang_masuk.id_barang inner join barang_keluar on barang_keluar.id_barang=barang_masuk.id_barang where barang_masuk.id_barang='$id_barang' group by barang_masuk.id_barang;");
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <td align="right" width="25%"><div class="mb-1">
        <form class="d-flex" action="barangkeluarstf.php" method="post">
          ...
          <tbody>
            ...
            <tr align="center">
                ...
                <td>
                  ...
                  <button class=" btn btn-secondary" data-bs-toggle="modal" data-bs-target="#edit<?=$id_barangkeluar;?>"><img src="edit.png" width="15px" height="15px"></button>&emsp;
                  <button class="btn btn-danger"data-bs-toggle="modal" data-bs-target="#delete<?=$id_barangkeluar;?>"><img src="hapus.png" width="15px" height="15px"></button>
