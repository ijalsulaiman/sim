...
include 'koneksi.php';
  ...
  if($_SESSION['level']=="mgr"){
   echo "<script>alert('Anda Tidak Memiliki Akses');window.location='persediaanmgr.php';</script>;";
  }
  ...
  if($_SESSION['level']=="pcs"){
    echo "<script>alert('Anda Tidak Memiliki Akses');window.location='persediaanpcs.php';</script>;";
  }
	if(isset($_POST['tambahpersediaan'])){
     ...
     $tambahpersediaan = mysqli_query($koneksi,"insert into persediaan_barang (id_persediaan, id_barang, stok, satuan, id_gudang, lokasi, safety_stock) values('','$id_barang','$stok','$satuan','$id_gudang','1')");
    $tambahrop = mysqli_query($koneksi,"insert into perencanaan (id_perencanaan, id_barang, rop) values('','$id_barang','1')");
      if (!$tambahpersediaan) {
      echo "<script>alert('Pastikan semua kolom terisi dengan benar! ');window.location='persediaanadm.php';</script>;";
    }else{
      echo "<script>alert('Data Berhasil Ditambahkan');window.location='persediaanadm.php';</script>;";
    }
}
if(isset($_POST['updatepersediaan'])){
    ...
    $updatepersediaan = mysqli_query($koneksi,"update persediaan_barang set safety_stock='$safety_stock', stok='$stok', id_gudang='$id_gudang', safety_stock='$safety_stock', lokasi='$lokasi' where id_barang='$id_barang'");
    if (!$updatepersediaan) {
      echo "<script>alert('Pastikan semua kolom terisi dengan benar! ');window.location='persediaanadm.php';</script>;";
    }else{
      echo "<script>alert('Data Berhasil Diubah');window.location='persediaanadm.php';</script>;";
    }
   }
      ...
      <div class="container-fluid">
		<h1 align="center">PERSEDIAAN BARANG</h1>
			...
			<td align="right" width="25%"><div class="mb-1">
				<form class="d-flex" action="persediaanadm.php" method="post">
